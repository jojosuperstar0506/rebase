...
| F10 | **CI: extend the scrape + analysis pipeline beyond bags (footwear / apparel / beauty / etc.)** | B | 2026-05-22 | The onboarding wizard (v2) now lets users in ANY vertical sign up — Bags, Footwear, Apparel, Beauty, Jewelry, Home. But the data pipeline only actually works for **bags (女包)** today. Non-bag signups are captured correctly but cannot produce a real Brief until this is done. Be explicit about the three layers: **(1) GET / scrape** — `services/competitor_intel/` scrapers are bag-tuned: the XHS/Douyin keyword strategies, Tmall category crawl paths, and `backend/brand_registry.js` (`KNOWN_BRANDS`, only 女包 brands) all assume handbags. Each new vertical needs its own platform keyword map + crawl config; brand_registry should become per-vertical (or be retired in favour of the AI-suggest path). **(2) STORE** — workspace category is captured in `workspaces.brand_category_l1` + `brand_subcategories` (migration 011). `scraped_products` / `scraped_brand_profiles` are schema-agnostic and need no change, but `material_tags` / dimension fields are bag-specific and should be generalised. **(3) SHOW / analyse** — `services/competitor_intel/composite_indices.py` `INDEX_HIERARCHY` weighting and the 7-dimension scoring are bag-centric; each vertical needs its own dimension weighting (e.g. footwear cares about comfort/durability, beauty about ingredients/efficacy). Frontend (CIBrief, CIAnalytics, CICompetitors) renders generically and needs little change. **Until F10 ships, only Bags workspaces get real briefs** — other verticals show the onboarding-complete state but an empty/pending Brief. |
